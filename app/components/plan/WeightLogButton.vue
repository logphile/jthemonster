...
<template>
  <div>
    <button class="px-3 py-2 rounded-xl bg-red-600 hover:bg-red-500 text-sm font-semibold" @click="open=true">Log Weight</button>
    <div v-if="open" class="fixed inset-0 z-40 grid place-items-end sm:place-items-center">
      ...
      <div class="relative z-10 w-full sm:max-w-sm rounded-t-2xl sm:rounded-2xl bg-card border border-border/60 p-4 backdrop-blur">
        ...
        <div class="mt-3 flex justify-end gap-2">
          ...
          <button class="px-3 py-2 rounded-xl bg-red-600 hover:bg-red-500" @click="save">Save</button>
        </div>
      </div>
    </div>
  </div>
