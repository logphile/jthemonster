<template>
  <div class="p-4">
    ...
    <div class="flex gap-2 mb-3">
      <input v-model="name" placeholder="New exercise" class="input" />
    ...
    <ul>
      <li v-for="e in exercises.list" :key="e.id" class="py-2 flex justify-between items-center">
        <span>{{ e.name }}</span>
        <button class="text-sm text-textLo hover:text-firepink-600 transition" @click="exercises.remove(e.id)">Delete</button>
      </li>
    </ul>
  </div>
</template>
